...
공개 저장소만 단독으로 받은 경우에는 `project_ready_data`가 비어 있거나 없을 수 있으니,  
그럴 때는 결과 확인 중심으로 먼저 읽고 전체 산출물은 `프로젝트-공유패키지` 쪽을 함께 보는 편이 안전하다.
...
- 이 저장소에는 실험 중간 산출물과 최종 산출물이 함께 섞여 있다.
- 처음에는 `core10`으로 시작하는 최종 결과 파일을 우선 보는 편이 덜 헷갈린다.
- 공개 저장소에는 대용량 생성 결과가 모두 포함되지 않을 수 있으니, 전체 산출물이 필요하면 `프로젝트-공유패키지`를 함께 확인해야 한다.
- 재현보다 결과 이해가 목적이라면 `README.md → RESULTS.md → FINAL_MODEL.md → 메인 노트북` 순서로 보는 것이 가장 빠르다.
